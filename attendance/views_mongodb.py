...
def dashboard(request):
    ...
    present_records = attendance_db.count({'sabha_id': {'$in': user_sabha_ids}, 'status': 'present'}) if user_sabha_ids else 0
    ...
    this_week_sabhas = sabhas_db.count({
        'sabha_type': {'$in': allowed_sabha_types},
        'date': {'$gte': week_start.isoformat(), '$lte': week_end.isoformat()}
    }) if allowed_sabha_types else 0
...
def devotee_edit(request, pk):
    ...
    if devotee.get('sabha_type') not in allowed_sabha_types:
...
def mark_attendance(request, sabha_id):
    ...
    if request.method == 'POST':
        ...
        # Send SMS to absent devotees if requested
        if request.POST.get('send_sms'):
            ...
            for devotee in devotees:
                ...
                if status == 'absent':
                    absent_count += 1
                    # Create devotee object for SMS
                    class DevoteeObj:
                        def __init__(self, data):
                            self.name = data['name']
                            self.contact_number = data['contact_number']
                    
                    devotee_obj = DevoteeObj(devotee)
                    
                    # Create sabha object for SMS
                    class SabhaObj:
                        def __init__(self, data):
                            self.date = data['date']
                            self.sabha_type = data['sabha_type']
                        def get_sabha_type_display(self):
                            return self.sabha_type.title() + ' Sabha'
                    
                    sabha_obj = SabhaObj(sabha)
                    
                    success, msg = send_absence_notification(devotee_obj, sabha_obj, request.user.username)
                    if success:
                        sms_sent += 1
            
            if absent_count > 0:
                messages.success(request, f'Attendance marked successfully! SMS sent to {sms_sent}/{absent_count} absent devotees.')
            else:
                messages.success(request, 'Attendance marked successfully! No absent devotees found.')
        else:
